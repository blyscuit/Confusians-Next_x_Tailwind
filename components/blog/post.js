...
export default function Post(props) {
  ...
  return (
    <div className="dark container mx-auto md:max-w-xl px-4 md:px-0">
      ...
      <div>
        {isDetail ? (
          <h1 className="font-sans-serif font-medium text-3xl text-gray-800 dark:text-white">
            {title}
          </h1>
        ) : (
          <Link
            href={{ pathname: "/blog/[blog]", query: { id } }}
            as={`/blog/${title}`}
          >
            <h1 className="font-sans-serif cursor-pointer font-medium text-3xl text-gray-800 dark:text-white">
              {title}
            </h1>
          </Link>
        )}
        <p className="font-light text-sm pt-2 text-gray-500">{`${mo} ${da}, ${ye}`}</p>
        <div className="prose-lg lg:prose-xl prose-blue font-serif leading-relaxed text-gray-900 dark:text-white">
          <ReactMarkdown
            components={{
